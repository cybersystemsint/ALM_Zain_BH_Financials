...
public class FinanceReportServiceImpl implements FinancialReportService {
    ...
    public tb_FinancialReport calculateDepreciation(String serialNumber, BigDecimal adjustment, String username) {
        ...
        // Limit initial cost to 3 decimal places
        report.setInitialCost(report.getInitialCost().setScale(3, RoundingMode.HALF_UP));
        ...
        // Cap at useful life
        numberOfMonthsUtilised = Math.min(numberOfMonthsUtilised, report.getUsefulLifeMonths());
        ...
        if (report.getWriteOffDate() != null) {
            ...
            long monthsToWriteOff = YearMonth.from(d).until(YearMonth.from(lastDayOfWriteOffMonth),
                    java.time.temporal.ChronoUnit.MONTHS);
            numberOfMonthsUtilised = Math.min(numberOfMonthsUtilised, monthsToWriteOff);
        }
        // Calculate x = MD * NoMU
        ...
        BigDecimal netCost = report.getInitialCost()
                .subtract(accumulatedDepreciation).setScale(3, RoundingMode.HALF_UP);
        ...
        if (netCost.compareTo(BigDecimal.ZERO) <= 0 || netCost.compareTo(report.getSalvageValue()) <= 0) {
            ...
            accumulatedDepreciation = report.getInitialCost().subtract(report.getSalvageValue()).setScale(3, RoundingMode.HALF_UP);
            report.setAccumulatedDepreciation(accumulatedDepreciation);
        }
    ...
    public Map<String, Object> calculateDepreciationForMonth(String date, String search, Pageable pageable) {
        try {
            ...
            // Format endOfMonth to String for query
            String endOfMonthStr = endOfMonthLocal.plusDays(1).format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            boolean isSearchEmpty = search == null || search.trim().isEmpty();
            // Fetch paginated subset
            Page<tb_FinancialReport> reports;
            if (isSearchEmpty) {
                reports = financialReportRepo.findByDateOfServiceBeforeNoSearch(endOfMonthStr, pageable);
            } else {
                reports = financialReportRepo.findByDateOfServiceBefore(endOfMonthStr, search, pageable);
            }
            ...
            CompletableFuture<BigDecimal> totalCostFuture = CompletableFuture.supplyAsync(() -> {
                if (isSearchEmpty) {
                    return financialReportRepo.findTotalCostByDateOfServiceBeforeNoSearch(endOfMonthStr);
                } else {
                    return financialReportRepo.findTotalCostByDateOfServiceBefore(endOfMonthStr, search);
                }
            });
            CompletableFuture<BigDecimal> totalDepreciationFuture = CompletableFuture.supplyAsync(() -> {
                if (isSearchEmpty) {
                    return financialReportRepo.findTotalDepreciationByDateOfServiceBeforeNoSearch(endOfMonthStr);
                } else {
                    return financialReportRepo.findTotalDepreciationByDateOfServiceBefore(endOfMonthStr, search);
                }
            });
            BigDecimal totalCost = totalCostFuture.get().setScale(3, RoundingMode.HALF_UP);
            BigDecimal totalDepreciation = totalDepreciationFuture.get().setScale(3, RoundingMode.HALF_UP);
            // Compute totalNBV as totalCost - totalDepreciation
            ...
            logger.error("Error in calculateDepreciationForMonth", e);
    ...
    @Override
    @Transactional(readOnly = true)
    public Map<String, Object> calculateDepreciationForMonthV2(String date, String search, Pageable pageable) {
        try {
            // Parse the input date
            SimpleDateFormat inputFormat = new SimpleDateFormat("d MMM yyyy");
            Date inputDate = inputFormat.parse(date);
            ...
            LocalDate localInputDate = inputDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
            LocalDate endOfMonthLocal = YearMonth.from(localInputDate).atEndOfMonth();
            // Format endOfMonth to String for dateOfService (String field)
            String endOfMonthStr = endOfMonthLocal.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            // Convert to Date for retirementDate (Date field) - assuming start of day for comparison
            Date retirementDate = Date.from(endOfMonthLocal.atStartOfDay(ZoneId.systemDefault()).toInstant());
            boolean isSearchEmpty = search == null || search.trim().isEmpty();

            // Fetch paginated subset
            Page<tb_FinancialReport> reports;
            if (isSearchEmpty) {
                reports = financialReportRepo.findByDateOfServiceBeforeAndRetirementDateIsNullOrAfter(
                        endOfMonthStr, "", retirementDate, pageable);
            } else {
                reports = financialReportRepo.findByDateOfServiceBeforeAndRetirementDateIsNullOrAfter(
                        endOfMonthStr, search, retirementDate, pageable);
            }

            // Check if the requested page is beyond the available data
            if (pageable.getPageNumber() >= reports.getTotalPages() || reports.getTotalElements() == 0) {
                Map<String, Object> result = new HashMap<>();
                result.put("filteredCost", BigDecimal.ZERO.setScale(3, RoundingMode.HALF_UP));
                result.put("filteredDepreciation", BigDecimal.ZERO.setScale(3, RoundingMode.HALF_UP));
                result.put("filteredMonthlyDepreciation", BigDecimal.ZERO.setScale(3, RoundingMode.HALF_UP));
                result.put("filteredNBV", BigDecimal.ZERO.setScale(3, RoundingMode.HALF_UP));
                result.put("totalCost", BigDecimal.ZERO.setScale(3, RoundingMode.HALF_UP));
                result.put("totalDepreciation", BigDecimal.ZERO.setScale(3, RoundingMode.HALF_UP));
                result.put("totalMonthlyDepreciation", BigDecimal.ZERO.setScale(3, RoundingMode.HALF_UP));
                result.put("totalNBV", BigDecimal.ZERO.setScale(3, RoundingMode.HALF_UP));
                result.put("totalPages", reports.getTotalPages());
                result.put("totalItems", reports.getTotalElements());
                result.put("currentPage", pageable.getPageNumber());
            ...
            CompletableFuture<BigDecimal> totalCostFuture = CompletableFuture.supplyAsync(() -> {
                if (isSearchEmpty) {
                    return financialReportRepo.findTotalCostByDateOfServiceBeforeAndRetirementDateIsNullOrAfter(endOfMonthStr, retirementDate);
                } else {
                    return financialReportRepo.findTotalCostByDateOfServiceBeforeAndRetirementDateIsNullOrAfter(endOfMonthStr, search, retirementDate);
                }
            });
            CompletableFuture<BigDecimal> totalDepreciationFuture = CompletableFuture.supplyAsync(() -> {
                if (isSearchEmpty) {
                    return financialReportRepo.findTotalDepreciationByDateOfServiceBeforeAndRetirementDateIsNullOrAfter(endOfMonthStr, retirementDate);
                } else {
                    return financialReportRepo.findTotalDepreciationByDateOfServiceBeforeAndRetirementDateIsNullOrAfter(endOfMonthStr, search, retirementDate);
                }
            });
            CompletableFuture<BigDecimal> totalMonthlyDepreciationFuture = CompletableFuture.supplyAsync(() -> {
                if (isSearchEmpty) {
                    return financialReportRepo.findTotalMonthlyDepreciationByDateOfServiceBeforeAndRetirementDateIsNullOrAfter(endOfMonthStr, retirementDate);
                } else {
                    return financialReportRepo.findTotalMonthlyDepreciationByDateOfServiceBeforeAndRetirementDateIsNullOrAfter(endOfMonthStr, search, retirementDate);
                }
            });

            BigDecimal totalCost = totalCostFuture.get().setScale(3, RoundingMode.HALF_UP);
            BigDecimal totalDepreciation = totalDepreciationFuture.get().setScale(3, RoundingMode.HALF_UP);
            BigDecimal totalMonthlyDepreciation = totalMonthlyDepreciationFuture.get().setScale(3, RoundingMode.HALF_UP);
            BigDecimal totalNBV = totalCost.subtract(totalDepreciation).setScale(3, RoundingMode.HALF_UP);
            ...
            BigDecimal filteredDepreciation = filteredReports.stream()
                    ...
                    .reduce(BigDecimal.ZERO, BigDecimal::add)
                    .setScale(3, RoundingMode.HALF_UP);
            BigDecimal filteredMonthlyDepreciation = filteredReports.stream()
                    .map(tb_FinancialReport::getMonthlyDepreciationAmount)
                    .filter(Objects::nonNull)
                    .reduce(BigDecimal.ZERO, BigDecimal::add)
                    .setScale(3, RoundingMode.HALF_UP);
            BigDecimal filteredNBV = filteredCost.subtract(filteredDepreciation).setScale(3, RoundingMode.HALF_UP);
            ...
            result.put("filteredCost", filteredCost);
            result.put("filteredDepreciation", filteredDepreciation);
            result.put("filteredMonthlyDepreciation", filteredMonthlyDepreciation);
            result.put("filteredNBV", filteredNBV);
            result.put("totalCost", totalCost);
            result.put("totalDepreciation", totalDepreciation);
            result.put("totalMonthlyDepreciation", totalMonthlyDepreciation);
            result.put("totalNBV", totalNBV);
            result.put("totalPages", reports.getTotalPages());
            result.put("totalItems", reports.getTotalElements());
            result.put("currentPage", pageable.getPageNumber());
    ...
    @Override
    public Page<tb_FinancialReport> findByStatusFlagNot(String statusFlag, Pageable pageable) {
        return financialReportRepo.findByStatusFlagNot(statusFlag, pageable);
    }
    @Override
    ...
    public BigDecimal getTotalCost() {
        return financialReportRepo.findTotalCost().orElse(BigDecimal.ZERO).setScale(3, RoundingMode.HALF_UP);
    }
    @Override
    public BigDecimal getTotalNBV() {
        BigDecimal totalCost = getTotalCost();
        BigDecimal totalDepreciation = getTotalDepreciation();
        return totalCost.subtract(totalDepreciation).setScale(3, RoundingMode.HALF_UP);
    }
    @Override
    public BigDecimal getTotalDepreciation() {
        return financialReportRepo.findTotalDepreciation().orElse(BigDecimal.ZERO).setScale(3, RoundingMode.HALF_UP);
    }
    @Override
    public BigDecimal getFilteredCost(String search, String lastMonthDate) {
        return financialReportRepo.findFilteredCost(search, lastMonthDate).orElse(BigDecimal.ZERO).setScale(3, RoundingMode.HALF_UP);
    }
    @Override
    public BigDecimal getFilteredNBV(String search, String lastMonthDate) {
        BigDecimal filteredCost = getFilteredCost(search, lastMonthDate);
        BigDecimal filteredDepreciation = getFilteredDepreciation(search, lastMonthDate);
        return filteredCost.subtract(filteredDepreciation).setScale(3, RoundingMode.HALF_UP);
    }
    @Override
    public BigDecimal getFilteredDepreciation(String search, String lastMonthDate) {
        return financialReportRepo.findFilteredDepreciation(search, lastMonthDate).orElse(BigDecimal.ZERO).setScale(3, RoundingMode.HALF_UP);
    }
    @Override
    ...
    private BigDecimal calculateDepreciation(FinancialReportProjection p, Date endOfMonth) {
        ...
        try {
            ...
            LocalDate serviceLocal = serviceDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
            LocalDate endLocal = endOfMonth.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
            LocalDate d = serviceLocal.withDayOfMonth(1).plusMonths(1);
            long monthsActive = YearMonth.from(d).until(YearMonth.from(endLocal), ChronoUnit.MONTHS);
